...
def doEpisodeMC(obs, decay, i_episode):
    ...
    for t in range(200):
        ...
        obs, reward, done, _, _ = env.step(action)  # Apply the action
...
# @param decay <int>
# @return an action
# The policy progressively stops exploration and gets greedy
def policy(state, decay):
    ...
    if state not in history:  # If state does not exist, create it
        history[state] = []
